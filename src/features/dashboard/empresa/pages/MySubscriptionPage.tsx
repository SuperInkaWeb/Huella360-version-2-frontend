...
const PLAN_TYPE_BY_ROLE: Record<string, string> = {
    EMPRESA: "B2B",
    VETERINARIO: "B2B",
    CLIENTE: "B2C",
};
    ...
    const fetchSubscriptionData = async () => {
        try {
            ...
            const [allPlansResult, mySubResult, usageResult] = await Promise.allSettled([
                subscriptionService.getPlans(planType),
            ...
            html: `
                <div class="text-left space-y-4">
                    <div class="flex items-center justify-between text-lg font-bold">
                        ...
                        <span class="text-primary">S/ ${plan.precioMensual.toFixed(0)}<span class="text-sm font-normal text-slate-400">/mes</span></span>
                    </div>
            ...
            <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 p-4 md:p-6 grid grid-cols-2 sm:grid-cols-4 gap-4 shadow-sm">
                        ...
                        <StatCard icon={<ShieldCheck size={20} />} label="Estado" value={
                            <span className="flex items-center gap-1.5">
                                <span className={`w-2 h-2 rounded-full ${mySub?.estado === 'ACTIVA' ? 'bg-emerald-500' : 'bg-slate-300'}`} />
                                {mySub?.estado === 'ACTIVA' ? 'Activa' : mySub?.estado}
                            </span>
                        } />
                    </div>
                )}
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pb-4">
                        ...
                        {upgradePlans.map((plan) => (
                            <div key={plan.id} className="rounded-2xl border border-slate-200 bg-white p-6 md:p-8 flex flex-col hover:shadow-lg hover:border-slate-300 transition-all">
                                ...
                                <div className="flex items-baseline gap-1 mb-6">
                                    <span className="text-3xl font-black text-slate-900">S/ {plan.precioMensual.toFixed(0)}</span>
                                    <span className="text-xs text-slate-400 font-bold uppercase">/mes</span>
                                </div>
                                <div className="space-y-3 mb-8 flex-1 text-sm">
                ...
                {isMaxPlan && isPaidPlan && (
                    <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-2xl p-8 text-white text-center">
                        <p className="text-lg font-bold">Tienes el plan más completo. ¡Disfruta de todos los beneficios!</p>
                    </div>
                )}
